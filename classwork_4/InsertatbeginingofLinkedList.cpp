...
void push(Node** head_ref, int new_data)
{
    ...
    // 4. Update the head pointer to point to the new node, effectively making it the new head of the list
    (*head_ref) = new_node;
}
...
int main() {
    ...
    push(&head, 4);   // Add a node with data 4
    push(&head, 9);   // Add a node with data 9
    push(&head, 7);   // Add a node with data 7
